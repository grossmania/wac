...
Q1.fac<-all.data %>% select(q1)%>% mutate_at(q1, funs(factor(.)))

names(Q1.fac)<-Q1.q


# number of categories per variable
cats = apply(Q1.fac, 2, function(x) nlevels(as.factor(x)))
cats
...
q1.eig.val <- factoextra::get_eigenvalue(q1.mca)
 # head(eig.val)

q1.var<-get_mca_var(q1.mca)
q1.var$cos2 #quality of representation of variable categories
round(q1.var$contrib,2) #Contribution of variable categories to the dimensions


#To visualize the percentages of themes explained by each MCA dimensions, use the function fviz_eig() or fviz_screeplot() [factoextra package]:
fviz_screeplot(q1.mca, choice="variance",addlabels = TRUE, geom="line") #does not look like there is much reduction happening...
...
edge.start <- ends(Q1.network, es=E(Q1.network), names=F)[,1]
edge.col <- V(Q1.network)$color[edge.start]

plot(Q1.ceb, Q1.network, axes=F, ,vertex.shape="none",edge.color=edge.col, edge.curved=.1,
     vertex.label.color="black", vertex.label.font=1,vertex.label.dist=0.5,layout=layout_with_fr(Q1.network)) 
...
Q3.q <- c("Estrangement Alienation","Career Disruptions", "Low Qual. Education", "Low Trust in Science","Low Wellbeing"   ,                     
...
Q3.SC.plot<-ggplot(Q3.SC.summ, aes(x=Theme, y=Percent,fill=Type)) + coord_flip() + facet_grid(. ~ Field)+
  geom_bar(stat="identity", width=.5,position=position_dodge()) + ggtitle("Negative Consequences")+theme_pubclean()+scale_fill_lancet()+theme(plot.title = element_text(hjust = 0.5),legend.position="bottom")+labs(x="",y="%")
Q3.SC.plot
```
...
#first, convert to categorical
Q3.fac<-all.data %>% select(q3)%>%mutate_at(q3, funs(factor(.)))

names(Q3.fac)<-Q3.q

Q3.fac<-na.omit(Q3.fac) #gt rid of NAs
...
#To visualize the percentages of themes explained by each MCA dimensions, use the function fviz_eig() or fviz_screeplot() [factoextra package]:
fviz_screeplot(q3.mca, choice="variance",addlabels = TRUE, geom="line") #does not look like there is much reduction happening...


q3.eig.val <- factoextra::get_eigenvalue(q3.mca)
 # head(eig.val)

q3.var<-get_mca_var(q3.mca)
round(q3.var$cos2,3) #quality of representation of variable categories
round(q3.var$contrib,3) #Contribution of variable categories to the dimensions
...
plot(Q3.ceb, Q3.network, axes=T, vertex.label.color="black", edge.width=seq(1,10)) 

edge.start <- ends(Q3.network, es=E(Q3.network), names=F)[,1]
edge.col <- V(Q3.network)$color[edge.start]

plot(Q3.ceb, Q3.network, axes=F, ,vertex.shape="none",edge.color=edge.col, edge.curved=.1,
     vertex.label.color="black", vertex.label.font=1,vertex.label.dist=0.5,layout=layout_with_fr(Q3.network)) 
...
temp.change.graph<-ggplot(temporal.change.TT, aes(x=Period, y=Percent, group = Theme, color=Theme, shape=Theme,fill=Theme)) + 
    geom_line( ) +
    geom_point(size=4) +facet_wrap(~Type, ncol=1,as.table = FALSE)+
   theme_nice()+scale_color_d3()+scale_fill_d3()+theme(legend.title=element_blank(), legend.position = "top")
temp.change.graph
...
#first, convert to categorical
Q2.fac<-all.data %>% select(q2)%>%mutate_at(q2, funs(factor(.)))

names(Q2.fac)<-Q2.q

Q2.fac<-na.omit(Q2.fac) #gt rid of NAs
...
#To visualize the percentages of themes explained by each MCA dimensions, use the function fviz_eig() or fviz_screeplot() [factoextra package]:
fviz_screeplot(q2.mca, choice="variance",addlabels = TRUE, geom="line") #does not look like there is much reduction happening...


q2.eig.val <- factoextra::get_eigenvalue(q2.mca)
 # head(eig.val)

q2.var<-get_mca_var(q2.mca)
round(q2.var$cos2,2) #quality of representation of variable categories
round(q2.var$contrib,2) #Contribution of variable categories to the dimensions
...
#first, convert to categorical
q4.fac<-all.data %>% select(q4)%>%mutate_at(q4, funs(factor(.)))

names(q4.fac)<-Q4.q

q4.fac<-na.omit(q4.fac) #gt rid of NAs
...
#To visualize the percentages of themes explained by each MCA dimensions, use the function fviz_eig() or fviz_screeplot() [factoextra package]:
fviz_screeplot(q4.mca, choice="variance",addlabels = TRUE, geom="line") #does not look like there is much reduction happening...


q4.eig.val <- factoextra::get_eigenvalue(q4.mca)
 # head(eig.val)

q4.var<-get_mca_var(q4.mca)
round(q4.var$cos2,2) #quality of representation of variable categories
round(q4.var$contrib,2) #Contribution of variable categories to the dimensions
...
#first, convert to categorical
q5.fac<-all.data %>% select(q5)%>%mutate_at(q5, funs(factor(.)))

names(q5.fac)<-Q5.q

q5.fac<-na.omit(q5.fac) #gt rid of NAs
...
#To visualize the percentages of themes explained by each MCA dimensions, use the function fviz_eig() or fviz_screeplot() [factoextra package]:
fviz_screeplot(q5.mca, choice="variance",addlabels = TRUE, geom="line") #does not look like there is much reduction happening...


q5.eig.val <- factoextra::get_eigenvalue(q5.mca)
 # head(eig.val)

q5.var<-get_mca_var(q5.mca)
round(q5.var$cos2,2) #quality of representation of variable categories
round(q5.var$contrib,2) #Contribution of variable categories to the dimensions
...
colnames(data.245.j)<-c("Theme","Advice\nfor Positive","Advice\nagainst Negative","Advice for Now")
...
write.csv(Q12.cor.data,"corr12.csv") #R is weird with partial.r function so I just save and open the file again, stripping it to one layer
